...
up() ->
    ...
    case mnesia:create_schema([node()]) of
        ...
        ok -> 
            ...
            mnesia:create_table(info_application,
                [{type, ordered_set}
                ,{disc_copies, [node()]}
                ,{attributes, record_info(fields, info_application)}
                ]),
            ...
            mnesia:create_table(info_function,
                [{type, ordered_set}
                ,{disc_copies, [node()]}
                ,{attributes, record_info(fields, info_function)}
                ]),

            Res = mnesia:create_table(i_counter,
                [{type, set}
                ,{disc_copies, [node()]}
                ,{attributes, record_info(fields, i_counter)}
                ]),

            case Res of
                ok -> 
                    [mnesia:write(T, #i_counter{table = T, last_id = 0})
                     || T <- tables(), T =/= i_counter];
                _Other ->
...
next_id(Tab) ->
    PrevN = mnesia:dirty_update_counter(i_counter, Tab, #i_counter.last_id),
    PrevN + 1.
